...
// Formats class time for display (Today, Tomorrow, or date)
...
// Get capacity status based on enrollment percentage
...
// Extract an excerpt from feedback text
  ...
  return (
    ...
    >
      <StatLabel display="flex" alignItems="center">
        <Icon as={icon} mr={2} color="purple.500" />
        {label}
      </StatLabel>
      {isLoading ? (
...
const UpcomingClassesCard = ({
  ...
  // Calculate the number of bookings for a class
  const getEnrolledCount = (fitnessClass: FitnessClass) =>
    fitnessClass.bookings?.length || 0;
      ...
      <CardBody>
        {isLoading ? (
          ...
          <Box
            ...
            borderStyle="dashed"
          >
            <Icon as={FaCalendarAlt} boxSize={8} color="purple.500" mb={3} />
                  ...
                  <HStack justify="space-between" mb={3}>
                    ...
                    <HStack>
                      ...
                      <Text fontSize="sm" fontWeight="medium">
                        {getClassDuration(classItem.startsAt, classItem.endsAt)}
                      </Text>
                    </HStack>
                  </HStack>
                    ...
                    {classItem.bookings === undefined ? (
                      ...
                      <>
                        <HStack justify="space-between" mb={1}>
                          ...
                          <Text fontSize="xs" fontWeight="medium">
                            {Math.round((enrolled / classItem.capacity) * 100)}%
                          </Text>
                        </HStack>
                        <Progress
...
// Component for displaying top students
interface Student {
  id: number;
  name: string;
  classes: number;
...
const TopStudentsCard = ({ students }: TopStudentsCardProps) => {
  const cardBg = useColorModeValue("white", "gray.800");
  const cardBorder = useColorModeValue("gray.200", "gray.700");
  const textColor = useColorModeValue("gray.600", "gray.300");
  const headingColor = useColorModeValue("gray.800", "white");
...
// Component for displaying student reviews
interface StudentReviewsCardProps {
  reviews: Review[];
  isLoading: boolean;
}

const StudentReviewsCard = ({
  reviews,
  isLoading,
}: StudentReviewsCardProps) => {
  const cardBg = useColorModeValue("white", "gray.800");
  const cardBorder = useColorModeValue("gray.200", "gray.700");
  const textColor = useColorModeValue("gray.600", "gray.300");
  const headingColor = useColorModeValue("gray.800", "white");
      ...
      <CardBody>
        {isLoading ? (
          ...
          <Box p={6} textAlign="center">
            <Text color={textColor}>No reviews yet for your classes.</Text>
            <Text fontSize="sm" mt={2}>
              Reviews will appear here when students leave feedback for your
              classes.
            </Text>
          </Box>
        )}
    ...
    >
      ...
      >
        ...
        <StatCard
          ...
          value={reviews.length ? `${calculateAverageRating()}/5` : "N/A"}
          helpText={
            reviews.length > 0
              ? `From ${reviews.length} ${
      ...
      <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={8} mb={8}>
        {/* Upcoming Classes and Top Students */}
        ...
        {/* Recent Feedback */}
        <MotionBox variants={itemVariants}>
          <StudentReviewsCard reviews={reviews} isLoading={isLoadingReviews} />
        </MotionBox>
      </SimpleGrid>
    </MotionBox>
  );
